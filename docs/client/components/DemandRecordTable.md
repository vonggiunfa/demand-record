...
├── SearchBar.tsx          # 搜索栏组件
├── TableHeader.tsx        # 表格标题（年月选择器/搜索结果状态）
├── TableBody.tsx          # 表格主体
...
- **SearchBar**：提供搜索功能，包括简单搜索和打开高级搜索
- **TableHeader**：显示年月选择器或搜索结果状态
...
2. **年月选择区域(TableHeader)**：
   - 年月选择器：选择要显示的年月数据
   - 搜索结果状态：显示搜索结果数量和返回按钮
...
## bug修复点：
- 把年月选择器，放置在搜索、导入导出同一行。减少视觉差异感。
- 移除对应废弃代码，比如executeSearch，既然移除了实际功能，就不要再保留，以免造成后续维护困难不够清晰。